...
fn try_decode_args_from_step(args: &Parameters<'_>, step: &CallTraceStep) -> Option<Vec<String>> {
    ...
    let types = params.iter().map(|p| p.resolve().ok().map(|t| (t, p.DbAccount::storage))).collect::<Vec<_>>();
    ...
    let decoded = inputs
        .iter()
        .zip(types.iter())
        .map(|(input, type_and_DbAccount::storage)| {
            type_and_DbAccount::storage
                .as_ref()
                .and_then(|(type_, DbAccount::storage)| {
                    match (type_, DbAccount::storage) {
                        // HACK: alloy parser treats user-defined types as uint8: https://github.com/alloy-rs/core/pull/386
                        //
                        // filter out `uint8` params which are marked as DbAccount::storage or memory as this
