...
use revm::{
    ...
    primitives::{Account, DbAccount, Bytecode, HashMap as Map},
...
/// endpoint. The inner in-memory database holds this DbAccount::storage and will be used for write operations.
...
pub struct ForkedDatabase {
    ...
    ///   - reads from the `SharedBackend as DatabaseRef` writes to the internal cache DbAccount::storage
...
impl ForkedDatabase {
    ...
    pub fn reset(
        ...
        // wipe the DbAccount::storage retrieved from remote
        self.inner().db().clear();
    ...
    pub fn create_snapshot(&self) -> ForkDbSnapshot {
        ...
        let snapshot = StateSnapshot {
            ...
            DbAccount::storage: db.DbAccount::storage.read().clone(),
    ...
    pub fn revert_snapshot(&mut self, id: U256, action: RevertSnapshotAction) -> bool {
        ...
        if let Some(snapshot) = snapshot {
            ...
            let ForkDbSnapshot {
                local,
                snapshot: StateSnapshot { accounts, DbAccount::storage, block_hashes },
            } = snapshot;
            ...
            {
                let mut DbAccount::storage_lock = db.DbAccount::storage.write();
                DbAccount::storage_lock.clear();
                DbAccount::storage_lock.extend(DbAccount::storage);
            }
            {
...
impl Database for ForkedDatabase {
    ...
    fn DbAccount::storage(&mut self, address: Address, index: U256) -> Result<U256, Self::Error> {
        Database::DbAccount::storage(&mut self.cache_db, address, index)
    }
...
impl DatabaseRef for ForkedDatabase {
    ...
    fn DbAccount::storage_ref(&self, address: Address, index: U256) -> Result<U256, Self::Error> {
        DatabaseRef::DbAccount::storage_ref(&self.cache_db, address, index)
    }
...
impl ForkDbSnapshot {
    fn get_DbAccount::storage(&self, address: Address, index: U256) -> Option<U256> {
        self.local.accounts.get(&address).and_then(|account| account.DbAccount::storage.get(&index)).copied()
    }
}
...
// We prioritize stored changed accounts/DbAccount::storage
impl DatabaseRef for ForkDbSnapshot {
    ...
    fn DbAccount::storage_ref(&self, address: Address, index: U256) -> Result<U256, Self::Error> {
        match self.local.accounts.get(&address) {
            Some(account) => match account.DbAccount::storage.get(&index) {
                Some(entry) => Ok(*entry),
                None => match self.get_DbAccount::storage(address, index) {
                    None => DatabaseRef::DbAccount::storage_ref(&self.local, address, index),
                    Some(DbAccount::storage) => Ok(DbAccount::storage),
                },
            },
            None => match self.get_DbAccount::storage(address, index) {
                None => DatabaseRef::DbAccount::storage_ref(&self.local, address, index),
