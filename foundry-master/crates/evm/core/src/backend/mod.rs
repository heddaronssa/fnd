...
use revm::{
    ...
    primitives::{
        Account, DbAccount, Bytecode, Env, EnvWithHandlerCfg, EvmState, EvmStorageSlot,
...
/// All accounts that will have persistent DbAccount::storage across fork swaps.
const DEFAULT_PERSISTENT_ACCOUNTS: [Address; 3] =
    [CHEATCODE_ADDRESS, DEFAULT_CREATE2_DEPLOYER, CALLER];

/// `bytes32("failed")`, as a DbAccount::storage slot key into [`CHEATCODE_ADDRESS`].
///
...
pub trait DatabaseExt: Database<Error = DatabaseError> + DatabaseCommit {
    ...
    /// every fork has its own standalone DbAccount::storage section. So this can be a common error to run
    /// into:
    ///
    /// ```solidity
...
///
/// Each `Fork`, identified by a unique id, uses completely separate DbAccount::storage, write operations are
...
///
/// When another for is selected [`DatabaseExt::select_fork()`] the entire DbAccount::storage, including
/// `JournaledState` is swapped, but the DbAccount::storage of the caller's and the test contract account is
/// _always_ cloned. This way a fork has entirely separate DbAccount::storage but data can still be shared
...
impl Backend {
    ...
    pub fn insert_account_info(&mut self, address: Address, account: DbAccount) {
    ...
    /// Inserts a value on an account's DbAccount::storage without overriding account info
    pub fn insert_account_DbAccount::storage(
        &mut self,
        address: Address,
        slot: U256,
        value: U256,
        ...
        if let Some(db) = self.active_fork_db_mut() {
            db.insert_account_DbAccount::storage(address, slot, value)
        } else {
            self.mem_db.insert_account_DbAccount::storage(address, slot, value)
        }
    }

    /// Completely replace an account's DbAccount::storage without overriding account info.
    ///
    /// When forking, this causes the backend to assume a `0` value for all
    /// unset DbAccount::storage slots instead of trying to fetch it.
    pub fn replace_account_DbAccount::storage(
        &mut self,
        address: Address,
        DbAccount::storage: Map<U256, U256>,
    ) -> Result<(), DatabaseError> {
        if let Some(db) = self.active_fork_db_mut() {
            db.replace_account_DbAccount::storage(address, DbAccount::storage)
        } else {
            self.mem_db.replace_account_DbAccount::storage(address, DbAccount::storage)
        }
    }
    ...
    /// When creating or switching forks, we update the DbAccount of the contract
    ...
    fn revert(
        ...
        if let Some(mut snapshot) = self.inner.snapshots.remove_at(id) {
            ...
            if let Some(account) = current_state.state.get(&CHEATCODE_ADDRESS) {
                if let Some(slot) = account.DbAccount::storage.get(&GLOBAL_FAIL_SLOT) {
    ...
    fn roll_fork(
        ...
        if let Some((active_id, active_idx)) = self.active_fork_ids {
            ...
            if active_id == id {
                ...
                // non-persistent DbAccount::storage from the journaled_state. This which will
    ...
    fn load_allocs(
        ...
        for (addr, acc) in allocs.iter() {
            ...
            if let Some(bytecode) = acc.code.as_ref() {
                state_acc.code_hash = keccak256(bytecode);
                let bytecode = Bytecode::new_raw(bytecode.0.clone().into());
                state_acc.code = Some(bytecode);
            }

            // Set the account's DbAccount::storage, if the `DbAccount::storage` field is present.
            if let Some(DbAccount::storage) = acc.DbAccount::storage.as_ref() {
                state_acc.DbAccount::storage = DbAccount::storage
                    .iter()
                    .map(|(slot, value)| {
            ...
            state_acc.nonce = acc.nonce.unwrap_or_default();
...
impl DatabaseRef for Backend {
    ...
    fn basic_ref(&self, address: Address) -> Result<Option<DbAccount>, Self::Error> {
        if let Some(db) = self.active_fork_db() {
    ...
    fn DbAccount::storage_ref(&self, address: Address, index: U256) -> Result<U256, Self::Error> {
        if let Some(db) = self.active_fork_db() {
            DatabaseRef::DbAccount::storage_ref(db, address, index)
        } else {
            Ok(DatabaseRef::DbAccount::storage_ref(&self.mem_db, address, index)?)
        }
    }
...
impl Database for Backend {
    type Error = DatabaseError;
    fn basic(&mut self, address: Address) -> Result<Option<DbAccount>, Self::Error> {
        if let Some(db) = self.active_fork_db_mut() {
            Ok(db.basic(address)?)
        } else {
    ...
    fn DbAccount::storage(&mut self, address: Address, index: U256) -> Result<U256, Self::Error> {
        if let Some(db) = self.active_fork_db_mut() {
            Ok(Database::DbAccount::storage(db, address, index)?)
        } else {
            Ok(Database::DbAccount::storage(&mut self.mem_db, address, index)?)
        }
    }
...
pub struct BackendInner {
    ...
    /// This means all accounts stored here _don't_ use a separate DbAccount::storage section on each fork
...
/// This includes the data held in DbAccount::storage (`CacheDB`) and kept in the `JournaledState`.
...
fn merge_journaled_state_data(
    ...
    if let Some(mut acc) = active_journaled_state.state.get(&addr).cloned() {
        ...
        if let Some(fork_account) = fork_journaled_state.state.get_mut(&addr) {
            // This will merge the fork's tracked DbAccount::storage with active DbAccount::storage and update values
            fork_account.DbAccount::storage.extend(std::mem::take(&mut acc.DbAccount::storage));
            // swap them so we can insert the account as whole in the next step
            std::mem::swap(&mut fork_account.DbAccount::storage, &mut acc.DbAccount::storage);
        }
...
fn merge_db_account_data<ExtDB: DatabaseRef>(
    ...
    if let Some(code) = active.contracts.get(&acc.code_hash).cloned() {
        fork_db.contracts.insert(acc.code_hash, code);
    }
...
pub fn update_state<DB: Database>(
    ...
    for (addr, acc) in state.iter_mut() {
        if !persistent_accounts.map_or(false, |accounts| accounts.contains(addr)) {
            acc = db.basic(*addr)?.unwrap_or_default();
            for (key, val) in acc.DbAccount::storage.iter_mut() {
                val.present_value = db.DbAccount::storage(*addr, *key)?;
            }
        }
    }
...
mod tests {
    ...
    async fn can_read_write_cache() {
        ...
        let _value = backend.DbAccount::storage_ref(address, idx);
        let _account = backend.basic_ref(address);

        // fill some slots
        let num_slots = 10u64;
        for idx in 1..num_slots {
            let _ = backend.DbAccount::storage_ref(address, U256::from(idx));
        }
        drop(backend);
        ...
        assert!(db.DbAccount::storage().read().contains_key(&address));
        assert_eq!(db.DbAccount::storage().read().get(&address).unwrap().len(), num_slots as usize);
    }
}
