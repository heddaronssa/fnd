...
impl DatabaseRef for MemDb {
    ...
    fn DbAccount::storage_ref(&self, address: Address, index: U256) -> Result<U256, Self::Error> {
        DatabaseRef::DbAccount::storage_ref(&self.inner, address, index)
    }
...
impl Database for MemDb {
    ...
    fn basic(&mut self, address: Address) -> Result<Option<DbAccount>, Self::Error> {
        // Note: this will always return `Some(DbAccount)`, See `EmptyDBWrapper`
    ...
    fn DbAccount::storage(&mut self, address: Address, index: U256) -> Result<U256, Self::Error> {
        Database::DbAccount::storage(&mut self.inner, address, index)
    }
...
///
/// This will also _always_ return `Some(DbAccount)`:
///
...
/// returning `Some` with this type, which will then insert a default [`DbAccount`] instead
...
impl DatabaseRef for EmptyDBWrapper {
    ...
    fn basic_ref(&self, _address: Address) -> Result<Option<DbAccount>, Self::Error> {
        // Note: this will always return `Some(DbAccount)`, for the reason explained above
        Ok(Some(DbAccount::default()))
    }
    ...
    fn DbAccount::storage_ref(&self, address: Address, index: U256) -> Result<U256, Self::Error> {
        Ok(self.0.DbAccount::storage_ref(address, index)?)
    }
...
mod tests {
    ...
    fn cache_db_insert_basic_non_existing() {
        ...
        // when fetching again, the `DbAccount` is still `None` because the state of the account
    ...
    /// Demonstrates that `Database::basic` for `MemDb` will always return the `DbAccount`
    #[test]
    fn mem_db_insert_basic_default() {
        ...
        let info = Database::basic(&mut db, address).unwrap();
        // We know info exists, as MemDb always returns `Some(DbAccount)` due to the
        // `EmptyDbWrapper`.
        assert!(info.is_some());
