...
use revm::{
    db::DatabaseRef,
    primitives::{
        Account, DbAccount, Bytecode, Env, EnvWithHandlerCfg, HashMap as Map, ResultAndState,
        SpecId,
    },
...
impl<'a> DatabaseRef for CowBackend<'a> {
    ...
    fn DbAccount::storage_ref(&self, address: Address, index: U256) -> Result<U256, Self::Error> {
        DatabaseRef::DbAccount::storage_ref(self.backend.as_ref(), address, index)
    }
...
impl<'a> Database for CowBackend<'a> {
    ...
    fn DbAccount::storage(&mut self, address: Address, index: U256) -> Result<U256, Self::Error> {
        DatabaseRef::DbAccount::storage_ref(self, address, index)
    }
