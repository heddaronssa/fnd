...
pub struct EvmOpts {
    ...
    /// Disables DbAccount::storage caching entirely.
    pub no_DbAccount::storage_caching: bool,
...
impl EvmOpts {
    ...
    ///
    /// DbAccount::storage caching for the [CreateFork] will be enabled if
    ///   - `fork_url` is present
    ///   - `fork_block_number` is present
    ///   - `StorageCachingConfig` allows the `fork_url` + chain ID pair
    ///   - DbAccount::storage is allowed (`no_DbAccount::storage_caching = false`)
    ///
    /// If all these criteria are met, then DbAccount::storage caching is enabled and DbAccount::storage info will be
    /// written to `<Config::foundry_cache_dir()>/<str(chainid)>/<block>/DbAccount::storage.json`.
    ///
    /// for `mainnet` and `--fork-block-number 14435000` on mac the corresponding DbAccount::storage cache will
    /// be at `~/.foundry/cache/mainnet/14435000/DbAccount::storage.json`.
