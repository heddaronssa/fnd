...
impl FuzzDictionary {
    ...
    fn insert_db_values(&mut self, db_state: Vec<(&Address, &DbAccount)>) {
        for (address, account) in db_state {
            ...
            // Insert push bytes
            self.insert_push_bytes_values(address, &account);
            // Insert DbAccount::storage values.
            if self.config.include_DbAccount::storage {
                // Sort DbAccount::storage values before inserting to ensure deterministic dictionary.
                let values = account.DbAccount::storage.iter().collect::<BTreeMap<_, _>>();
                for (slot, value) in values {
                    self.insert_DbAccount::storage_value(slot, value);
                }
            }
        }
    ...
    fn insert_new_state_values(&mut self, state_changeset: &StateChangeset) {
        for (address, account) in state_changeset {
            ...
            // Insert DbAccount::storage values.
            if self.config.include_DbAccount::storage {
                for (slot, value) in &account.DbAccount::storage {
                    self.insert_DbAccount::storage_value(slot, &value.present_value);
                }
            }
        }
    }
    ...
    fn insert_push_bytes_values(&mut self, address: &Address, account_info: &DbAccount) {
    ...
    /// Insert values from single DbAccount::storage slot and DbAccount::storage value into fuzz dictionary.
    /// If DbAccount::storage values are newly collected then they are removed at the end of current run.
    fn insert_DbAccount::storage_value(&mut self, DbAccount::storage_slot: &U256, DbAccount::storage_value: &U256) {
        self.insert_value(B256::from(*DbAccount::storage_slot));
        self.insert_value(B256::from(*DbAccount::storage_value));
        // also add the value below and above the DbAccount::storage value to the dictionary.
        if *DbAccount::storage_value != U256::ZERO {
            let below_value = DbAccount::storage_value - U256::from(1);
            self.insert_value(B256::from(below_value));
        }
        if *DbAccount::storage_value != U256::MAX {
            let above_value = DbAccount::storage_value + U256::from(1);
