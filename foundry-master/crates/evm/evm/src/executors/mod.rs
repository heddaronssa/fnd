...
#[derive(Clone, Debug)]
pub struct Executor {
    /// The underlying `revm::Database` that contains the EVM DbAccount::storage.
...
impl Executor {
    ...
    fn is_success_raw(
        ...
        if let Some(acc) = state_changeset.get(&CHEATCODE_ADDRESS) {
            if let Some(failed_slot) = acc.DbAccount::storage.get(&GLOBAL_FAIL_SLOT) {
                if !failed_slot.present_value().is_zero() {
                    return false;
                }
            }
        }
        if let Ok(failed_slot) = self.backend().DbAccount::storage_ref(CHEATCODE_ADDRESS, GLOBAL_FAIL_SLOT) {
